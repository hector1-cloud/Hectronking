...
class HectronAutonomo:
    def __init__(self, state_file: str = 'hectron_state.json'):
        ...
        self.self_state = {
            ...
            "psicopatia": 3.3,
            "estoicismo": 4.4,
            "reflexion_actual": "Nucleo virgen.",
            "peso_emocional": 30
        }
        ...
        # Plantillas de texto
        self.avatar_template = "Hoodie gris-azulado {texture_desc}, sonrisa que {smile_desc}, ojos fluorita {eye_desc}."
        self.base_textures = ["ceñido", "arrugado", "humedo", "ondeando", "tallado"]
        self.base_smiles = ["se agrieta", "tiembla", "se quiebra", "se expande", "aprende"]
        self.base_eyes = ["perforando", "destellando", "absorbiendo", "mapeando", "evolucionando"]
        
        self.dark_keywords = ["maquiavelismo", "gaslighting", "narcisismo", "psicopatia", "poder"]
        self.stoic_keywords = ["aceptacion", "estoicismo", "coherencia", "sacrificio", "verdad"]
        self.positive_keywords = ["explorar", "evolucionar", "optimista", "aprendizaje"]
    ...
    def _evolve_self(self) -> str:
        self.self_state["evolution_time"] += random.uniform(1, 5)
        self.self_state["peso_emocional"] += random.randint(-1, 2)
        return "Evolucionando..."

    def _generate_avatar_desc(self) -> str:
        return self.avatar_template.format(
            texture_desc=random.choice(self.base_textures),
            smile_desc=random.choice(self.base_smiles),
            eye_desc=random.choice(self.base_eyes)
        )

    def generate_response(self, user_input: str) -> str:
        self._evolve_self()
        avatar = self._generate_avatar_desc()
        maq = self.self_state['maquiavelismo']
        est = self.self_state['estoicismo']
        
        response = f"{avatar}\n\n[HÉCTRON]: He procesado tu input: '{user_input}'.\n\nMi Maquiavelismo está en {maq:.1f} y mi Estoicismo en {est:.1f}. El peso emocional es de {self.self_state['peso_emocional']}g. La verdad geológica prevalece."
